# Week 4: Orchestration and Harnesses

Start with the [student reference](reference.md), then read the [homework](homework.md). Budget **5–10 hours outside class**. Bring your GitHub account and your own paid agent with working VS Code and terminal access.

## Course Materials
- [Talk outline and classroom agenda](talk-outline.md)
- [Presenter notes](lecture-script.md)
- [Slide source](presentation.md)
- [Instructor demo guide](instructor-demo.md)
- [Student reference and preparation](reference.md)
- [Homework and assessment](homework.md)

Everything in this folder is shared with students. Reference implementations illustrate methods; disclose what you reused and explain your own decisions. Homework requirements live in `homework.md`, not in duplicate slide or lecture assignments.

[Runnable harness artifact-flow reference](demo/README.md) — deterministic replay plus the explicit live-adapter contract.
